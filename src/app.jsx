...
import {
  ...
  Globe, Mail, ExternalLink, Power, ArrowDown, FileText,
} from "lucide-react";
...
@media(max-width:920px){
  .hero{grid-template-columns:1fr}.nav{display:none}.mobtop{display:flex}.main{padding:16px 16px 80px}
  .tools{grid-template-columns:1fr 1fr}.opts.two{grid-template-columns:1fr}.kpirow{grid-template-columns:1fr 1fr !important}
}
@media(max-width:520px){.tools{grid-template-columns:1fr}.q-h{font-size:23px}}
...
const STYLE=["Talking head","Lifestyle / B-roll","Educational","Gym / action","Voiceover","Story-driven"];
const TONE=["Direct & intense","Premium / luxury","Educational","Real-talk","Contrarian","Faith / mindset"];
const STRUGGLE=["Writing hooks","Staying consistent","Ideas / what to post","Editing for retention","Converting to sales","Building an offer"];
...
const STEPS=[
  {key:"name",kind:"text",req:true,title:"First, who are we building for?",sub:"Your name or brand handle — the OS personalizes everything to you.",ph:"e.g. Michael, or @yourbrand"},
  ...
  {key:"offer",kind:"single",req:true,two:true,title:"What are you selling today?",opts:OFFERSTATUS},
  ...
  {key:"style",kind:"multi",req:true,two:true,title:"What content formats feel like you?",sub:"Select all that apply.",opts:STYLE},
  ...
  {key:"struggle",kind:"multi",req:true,two:true,title:"Where do you lose the most?",sub:"Be honest — this sets your priority fixes.",opts:STRUGGLE},
  ...
  {key:"known",kind:"text",req:true,title:"What do you want to be known for?",sub:"One line — this becomes the spine of your brand positioning.",ph:"e.g. helping busy men get lean without giving up their life"},
  {key:"take",kind:"textarea",req:true,title:"Give me your hottest take.",sub:"The contrarian belief your best content gets built on.",ph:"e.g. Most people don't need more discipline. They need a better system."},
...
function computeProfile(a){
  ...
  const fb=i(FOLLOWERS,a.followers), of=i(OFFERSTATUS,a.offer), fq=i(FREQ,a.freq);
  ...
  return {...a,niches,goals,niche:niches[0],goal:goals[0],subs,index,tier,avg,weekly,proj,
  ...
  return (
    <div className="wrap" style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"40px 22px"}}>
      <div style={{maxWidth:680,textAlign:"center"}}>
        ...
        <div className="eyebrow" style={{marginBottom:18}}>The Creator Operating System · Powered by Michael Caso AI</div>
        ...
        <div style={{display:"flex",gap:26,justifyContent:"center",marginTop:46,flexWrap:"wrap"}}>
          {[["Content","Hooks · reels · carousels · scripts"],["Ads & AI","Buy growth · automate · leverage"],["Business","Offers · funnels · real revenue"]].map(([a,b])=>(
            <div key={a} style={{textAlign:"left"}}><div className="disp" style={{fontWeight:700,fontSize:19}}>{a}</div><div style={{color:"var(--faint)",fontSize:12.5,marginTop:3}}>{b}</div></div>
          ))}
        </div>
      </div>
    </div>
...
function Assessment({initial,onDone}){
  ...
  const val=a[s.key];
  const ok = !s.req ? true : (s.kind==="multi" ? (val||[]).length>0 : !!(val&&(""+val).trim()));
...
const STUDIO=[
  {g:"Create",items:[
    ...
    {k:"caption",ic:MessageSquare,t:"Caption Lab",d:"Captions that hold watch time."},
...
function systemPrompt(P){
  return `You are Creator AI — the intelligence at the center of CreatorX, the operating system for creator-entrepreneurs. You are trained by Michael Caso.
...
const CONTENT_TYPES=[
  ...
  {t:"Informational / educational",goal:"Saves + authority",ic:BookOpen,
  ...
  {t:"Engagement post",goal:"Comments + algorithm boost",ic:MessageSquare,
  ...
  const tasks=[
    ...
    {id:"trial",ic:Play,title:"Post 1 trial reel",time:"10 min",why:"Test a new hook or angle on non-followers first. Keep what lands, scale it.",
     body:(<>
       ...
       <Block label="Today's test hook"><div className="line-copy"><span>{hooks[6].t}</span><CopyBtn text={hooks[6].t}/></div></Block>
...
function PlaybookView({P,open}){
  ...
  const captions=[
    `${P.take||"Most people don't need more motivation. They need a system."}\n\nThe ones who win in ${(P.niche||"this").toLowerCase()} aren't more talented — they're more consistent with a better framework.\n\nSave this. Comment "${first.toUpperCase()}" and I'll send you mine.`,
    `I almost quit ${(P.niche||"this").toLowerCase()} at 800 followers.\n\nThen I changed one thing: I stopped posting for the algorithm and started posting for the save.\n\nEverything changed. Here's the system 👇\n\nComment "SYSTEM" to get it.`,
    `Unpopular opinion: posting more isn't the answer.\n\nPosting better hooks is.\n\n3 reels with a great hook will beat 30 with a weak one every time.\n\nAgree? Drop a 🔥 below.`,
  ];
  const ctas=[`Comment "${first.toUpperCase()}" and I'll DM you the full breakdown.`,"Save this so you don't lose it.","Send this to someone who needs to hear it.","Follow for the part 2 tomorrow.","Drop your answer below 👇","Tap the link in my bio to get started.","DM me the word START to begin."];
  ...
  return (
    <>
      ...
      {tab==="captions" && <div style={{display:"grid",gap:11}}>{captions.map((c,i)=>(
        <div className="out-card" key={i}><div className="kit">{c}</div><div style={{marginTop:10}}><CopyBtn text={c} label="Copy caption"/></div></div>
      ))}</div>}

      {tab==="ctas" && <div style={{display:"grid",gap:9}}>{ctas.map((c,i)=>(<div className="line-copy" key={i}><span>{c}</span><CopyBtn text={c}/></div>))}</div>}

      {tab==="types" && <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:13}}>{CONTENT_TYPES.map((c,i)=>(
        <div className="panel panel-pad" key={i}>
          <div style={{display:"flex",gap:11,alignItems:"center",marginBottom:10}}><div className="tool-ic" style={{width:38,height:38}}><c.ic size={18}/></div><div><div style={{fontWeight:600,fontSize:15}}>{c.t}</div><div className="mono" style={{fontSize:10.5,color:"var(--accent)"}}>{c.goal.toUpperCase()}</div></div></div>
          <ul className="mini-list">{c.struct.map(s=><li key={s}>{s}</li>)}</ul>
          <div style={{marginTop:10,fontSize:12.5,color:"var(--muted)",fontStyle:"italic"}}>e.g. {c.ex}</div>
        </div>
      ))}</div>}
      ...
      {tab==="trending" && (<>
        ...
        <div className="eyebrow" style={{margin:"4px 0 10px"}}>Trending audio styles to look for</div>
        <div style={{display:"grid",gap:9,marginBottom:20}}>{TRENDING_AUDIO.map(([t,d],i)=>(<div className="row-card" key={i}><div style={{display:"flex",gap:12,alignItems:"center"}}><Music size={16} color="var(--accent)"/><div><div style={{fontWeight:600,fontSize:14}}>{t}</div><div style={{color:"var(--muted)",fontSize:12.5,marginTop:2}}>{d}</div></div></div></div>))}</div>
      ...
      {tab==="algo" && <div style={{display:"grid",gap:10}}>{ALGO.map(([t,d],i)=>(
...
function EmailFunnelView({P,open}){
  const seqs=[
    {k:"welcome",t:"Welcome sequence",d:"First 4 emails after someone joins your list. Build trust fast.",ic:Send},
    {k:"nurture",t:"Nurture sequence",d:"Ongoing value emails that warm leads toward your offer.",ic:MessageSquare},
    {k:"sales",t:"Sales sequence",d:"Direct emails that pitch your offer and handle objections.",ic:DollarSign},
    {k:"launch",t:"Launch sequence",d:"Multi-day campaign to sell a product with urgency.",ic:Rocket},
    {k:"winback",t:"Re-engagement",d:"Win back cold subscribers who stopped opening.",ic:RefreshCw},
  ];
  return (
    <>
      ...
      <p style={{color:"var(--muted)",fontSize:14.5,maxWidth:600,marginBottom:18}}>Social is rented; your email list is owned. These sequences turn captured leads into buyers automatically. Pick one and I'll write the full sequence in your voice.</p>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:12}}>
        {seqs.map(s=>(
          <div className="tool" key={s.k} onClick={()=>open("email")}><ArrowUpRight className="arr" size={16}/><div className="tool-ic"><s.ic size={19}/></div><h4 style={{fontSize:15,marginTop:12}}>{s.t}</h4><p>{s.d}</p></div>
        ))}
      </div>
    </>
  );
...
function DistributionView({P,open}){
  const{n}=NW(P);
  const targets=[
    ["Instagram Reel","Vertical, hook in frame 1, captions on","Film,#E1306C"],
    ["TikTok","Same cut, louder hook, trending sound","Music"],
    ["YouTube Shorts","Add a punchy title card, loop the end","Play"],
    ["YouTube long-form","Expand into a full breakdown video","Play"],
    ["X / Twitter thread","Turn the points into a bookmarkable thread","MessageSquare"],
    ["LinkedIn post","Reframe as a B2B authority lesson","Briefcase"],
    ["Threads","Casual, conversational version of the hook","MessageCircle"],
    ["Facebook Reel","Repost native for the older demographic","Users"],
    ["Pinterest","Static idea pin with the key takeaway","Bookmark"],
    ["Email","One-idea email to your list with a CTA","Send"],
    ["Blog / SEO article","Long-form written version for search","Type"],
    ["Carousel","Slide-by-slide version for saves","Layers"],
  ];
  return (
    <>
      <div className="eyebrow">Multi-Platform Distribution</div>
      <h1 className="disp" style={{fontWeight:700,fontSize:28,margin:"6px 0 6px"}}>One video becomes twenty assets.</h1>
      <p style={{color:"var(--muted)",fontSize:14.5,maxWidth:600,marginBottom:18}}>Stop making content for one platform. Film once, then atomize it across everywhere your audience lives — each one re-cut natively, not copy-pasted. Drop in your reel and I'll generate every version.</p>
      <div className="banner" style={{marginBottom:16}}>
        <div style={{display:"flex",gap:12,alignItems:"center"}}><div className="tool-ic" style={{width:40,height:40}}><Repeat size={19}/></div><div><div style={{fontWeight:600,fontSize:14.5}}>Start with one piece of content</div><div style={{color:"var(--muted)",fontSize:13,marginTop:2}}>A reel, a transcript, a voice memo — anything.</div></div></div>
        <button className="btn btn-grad" style={{padding:"11px 18px",fontSize:14}} onClick={()=>open("repurpose")}>Repurpose now</button>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:12}}>
        {targets.map(([t,d],i)=>(
          <div className="panel panel-pad" key={i} style={{cursor:"pointer"}} onClick={()=>open("repurpose")}>
            <div style={{fontWeight:600,fontSize:14}}>{t}</div>
            <div style={{color:"var(--muted)",fontSize:12.5,marginTop:5,lineHeight:1.4}}>{d}</div>
          </div>))}
      </div>
    </>
  );
...
function PageScanView({P,go,open}){
  ...
  const [handle,setHandle]=useState(mode==="mine"?(P.igHandle||""):"");
  const [shot,setShot]=useState(null);
  const [loading,setLoading]=useState(false);
    ...
    <>
      ...
      <div style={{display:"grid",gridTemplateColumns:"260px 1fr",gap:18,marginTop:14}} className="hero">
        ...
        <div className="panel panel-pad">
          ...
          <DocActions title={`${m.t} — your build plan`} P={P} label="Build it for me — editable Word / PDF"
            blocks={[{label:"Model",text:`${m.t}  (${m.price})`},{label:"Why this works",text:m.best},
              {label:"What to build — step by step",text:m.build.map((s,i)=>`${i+1}. ${s}`).join("\n")},
              {label:"Pricing",text:m.price},
              {label:"Path to your first $10k",text:m.first},
              {label:"How to launch it",text:`1. Tease the outcome (not the offer) in a reel.\n2. Run a story poll to find interested people.\n3. DM everyone who engages and qualify them.\n4. Send the offer + a clear next step.\n5. Follow up for 10 days — the fortune is in the follow-up.`}]}/>
...
function GrowthEngineView({P}){
  ...
  const userPlats=(P.platforms&&P.platforms.length?P.platforms.filter(p=>plats.includes(p)):plats);
  const [tab,setTab]=useState(userPlats[0]||"Instagram");
  const a=PLAT_ACADEMY[tab];
  const hotNow=useMemo(()=>shuffleSeeded(a.hot,daySeed()).slice(0,4),[a]);
  return (
    <>
      ...
      <div className="panel panel-pad" style={{marginBottom:16,border:"1px solid rgba(245,158,11,.25)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8,marginBottom:12}}>
          ...
          <span className="mono" style={{fontSize:10.5,color:"var(--faint)"}}>updated daily · live trend feed in production</span>
        </div>
...
function Dashboard({P,onReassess,onUpdate}){
  ...
  const runAgent=(text)=>{
    ...
    if(a.type==="checkin"){ setCheckedIn(true); setStreak(s=>s+1); showToast("Check-in logged"); return "Logged this week's check-in — your pace just recalculated."; }
  ...
  return (
    <div className="wrap shell">
        ...
        {view==="ask" && (
          <div className="panel" style={{display:"flex",flexDirection:"column",height:"calc(100vh - 150px)",minHeight:460,overflow:"hidden"}}>
            <div style={{borderBottom:"1px solid var(--line)",padding:"16px 20px",display:"flex",alignItems:"center",gap:11}}>
              ...
              <div><div className="disp" style={{fontWeight:700,fontSize:16}}>Creator AI</div><div className="live" style={{fontSize:10.5}}><span className="pulse"/>trained by Michael Caso</div></div>
            </div>
            <ChatPanel P={P} msgs={chatMsgs} setMsgs={setChatMsgs} agent={runAgent}/>
          </div>
        )}
        {view==="today" && <TodayView P={P} open={setDrawer} go={setView} onComplete={()=>{setCheckedIn(true);setStreak(s=>s+1);showToast("Today logged");}}/>}
        {view==="home" && <Home P={P} open={setDrawer} go={setView} streak={streak} checkedIn={checkedIn}
          onCheckIn={()=>{setCheckedIn(true);setStreak(s=>s+1);}} ig={igCtx} openChat={openChat}/>}
        {view==="index" && <IndexView P={P}/>}
        {view==="tracker" && <Tracker P={P} ig={igCtx} streak={streak} checkedIn={checkedIn} onCheckIn={()=>{setCheckedIn(true);setStreak(s=>s+1);}}/>}
      ...
      {drawer && <Drawer P={P} k={drawer} close={()=>setDrawer(null)} onSave={(t)=>saveAsset(t||(toolTitle(drawer)+" draft"))} askAI={(prompt)=>{setDrawer(null);openChat(prompt);}}/>}
...
function Home({P,open,go,streak,checkedIn,onCheckIn,ig,openChat}){
  ...
  return (
    <>
      ...
      {/* four pillars */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:18}} className="kpirow">
        {[["Content","Create",Film,"studio","Hooks · reels · products"],["Growth","Grow",TrendingUp,"growth","Algorithms · ads · scan"],["Monetization","Earn",DollarSign,"business","Offers · funnels · revenue"],["Automation","Automate",Zap,"automation","DMs · email · flows"]].map(([t,verb,Ic,dest,d])=>(
          <div key={t} className="pillar-card" onClick={()=>go(dest)}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div className="tool-ic" style={{width:34,height:34}}><Ic size={16}/></div><ArrowUpRight size={15} color="var(--faint)"/></div>
            <div style={{fontWeight:700,fontSize:15,marginTop:10}}>{t}</div>
  ...
  return (
    <>
      ...
      <div className="nba" style={{marginBottom:16}}>
        ...
        <button className="btn btn-grad" style={{marginTop:14,padding:"11px 18px",fontSize:14}} onClick={()=>open("bridge")}>Bridge {strong.pl} → {weak.pl} <ArrowRight size={16} style={{verticalAlign:"-3px"}}/></button>
      </div>
...
function buildPool(templates,P){
  ...
  // dedupe + light shuffle for variety while keeping adjacency-different
  const seen=new Set(), uniq=[];
...
/* ---- CTA by goal ---- */
const CTA_T={
  "Grow followers":["Follow for part 2 tomorrow.","Follow so you don't miss the next one.","Follow if you want to {desire}.","Hit follow — I post this daily.","Follow for more {n} breakdowns.","Tap follow, your future self will thank you.","Follow — this is how you {desire}.","Follow if you're done with {pain}."],
  "Get saves":["Save this before you forget.","Save it — you'll want this later.","Save this for your next post.","Bookmark this and run it this week.","Save it so you can come back.","Save this if you want to {desire}.","Save this for the days you want to quit."],
  "Get shares":["Send this to someone who needs it.","Share this with someone starting {n}.","Tag a creator who needs to see this.","Share this to your story.","Send this to your group chat.","Share if you {desire} too.","Tag someone who's tired of {pain}."],
  "Drive DMs":['Comment "{W}" and I\'ll DM you the breakdown.','Comment "{W}" for the full guide.','Comment "{W}" and I\'ll send the template.','DM me "{W}" to get it.','Comment "{W}" — I\'ll send it over.','Comment "{W}" if you want the system.'],
  "Sell an offer":["DM me START to get the program.","Link in bio to start today.","Comment \"{W}\" and I'll send the details.","DM me if you want me to build this with you.","Spots are limited — DM to claim yours.","DM \"{W}\" if you're ready to {desire}."],
};
function ctaPool(P,goal){const t=CTA_T[goal]||["Save & follow for more."];return buildPool(t,P);}
...
const CONFIGS={
  email:{title:"Email Sequence Builder",intro:"Pick a sequence — I'll write every email in your voice.",steps:[
    ...
    {id:"goal",q:"What's the end goal?",kind:"select",options:(a,P)=>[O("Get them to know & trust me"),O("Drive them to "+((P.offer&&P.offer!=="Nothing yet")?P.offer:"my offer")),O("Book a call"),O("Buy a product")]},
  ],assemble:(a,P)=>{const{n,W}=NW(P);const type=txt(a.type),goal=txt(a.goal);const pk=pack(P);
    const emails=type.includes("Welcome")?[
      ["Email 1 · Welcome + quick win",`Subject: You're in 🎉 (start here)\n\nWelcome! You joined because ${pk.pains[0]} — I've got you.\n\nHere's your fastest win: ${_cap(pk.topics[0])}.\n\nDo that today. Tomorrow I'll show you the mistake most people make.`],
      ["Email 2 · The big mistake",`Subject: the #1 reason ${pk.pains[1]}\n\nMost people in ${n} keep ${pk.mistakes[0]}. That's the whole problem.\n\nInstead: ${_cap(pk.topics[1])}.\n\nReply and tell me where you're stuck — I read every one.`],
      ["Email 3 · Proof + story",`Subject: how I went from stuck to results\n\nQuick story... [your origin]. The turning point was when I stopped ${pk.mistakes[1]}.\n\nIf you want the same, here's the next step ↓`],
      ["Email 4 · Soft pitch",`Subject: want me to just hand you the system?\n\nIf you're serious about ${pk.desires[0]}, ${(P.offer&&P.offer!=="Nothing yet")?P.offer:"my program"} is the shortcut.\n\n[link] — let's go.`],
    ]:type.includes("Sales")?[
      ["Email 1 · Problem",`Subject: ${_cap(pk.pains[0])}? read this\n\nName the pain, agitate it, promise the fix. Tease the offer.`],
      ["Email 2 · Solution + offer",`Subject: the fix (and how to get it)\n\nIntroduce ${(P.offer&&P.offer!=="Nothing yet")?P.offer:"the offer"}, what's included, who it's for. Clear CTA.`],
      ["Email 3 · Objections",`Subject: "but what if..."\n\nHandle the top 3 objections. Add proof. Restate the CTA.`],
      ["Email 4 · Urgency / close",`Subject: closing soon\n\nDeadline or bonus expiring. Final call. One link.`],
    ]:type.includes("Launch")?[
      ["Day 1 · Tease",`Subject: something's coming...\n\nHint at the transformation. Build curiosity. No link yet.`],
      ["Day 2 · Reveal",`Subject: it's here\n\nReveal ${(P.offer&&P.offer!=="Nothing yet")?P.offer:"the offer"} + the promise. Open cart. CTA.`],
      ["Day 3 · Proof",`Subject: real results\n\nTestimonials / your own results. Restate offer. CTA.`],
      ["Day 4 · Last call",`Subject: closing tonight\n\nUrgency, scarcity, final CTA.`],
    ]:type.includes("Re-engage")?[
      ["Email 1 · Pattern interrupt",`Subject: did I lose you?\n\nHonest, short. Ask if they still want ${n} content. One link.`],
      ["Email 2 · Best hit",`Subject: in case you missed it\n\nSend your single best resource. Re-earn the open.`],
      ["Email 3 · Last email",`Subject: should I stop emailing you?\n\nClear opt-down. The ones who stay are your real list.`],
    ]:[
      ["Email 1 · Value",`Subject: ${_cap(pk.topics[0])}\n\nOne idea, well taught. No pitch. Build the habit of opening you.`],
      ["Email 2 · Value + lean-in",`Subject: ${_cap(pk.topics[1])}\n\nTeach, then mention what you do at the end.`],
      ["Email 3 · Value → CTA",`Subject: ${_cap(pk.topics[2])}\n\nTeach, then point to ${goal.toLowerCase()}.`],
    ];
    const text=emails.map(e=>`【 ${e[0]} 】\n${e[1]}`).join("\n\n");
    return{title:type,subtitle:`Goal: ${goal}`,blocks:emails.map(e=>({label:e[0],text:e[1]})),kit:text,docOpts:{subtitle:`${type} · goal: ${goal}`}};}},
  ...
  reels:{title:"Reel Builder",intro:"Build a custom reel — choose each beat, keep refreshing, I assemble it.",steps:[
    ...
    {id:"hook",q:"Pick your hook (first 3 seconds)",kind:"select",pool:(a,P)=>buildPool(HOOK_T,P),ai:(a,P)=>`Give me 15 more scroll-stopping hooks for a ${NW(P).n} reel, in my voice.`},
    {id:"retain",q:"Pick your retention beat (3–10s)",kind:"select",pool:(a,P)=>buildPool(RETAIN_T,P),ai:(a,P)=>`Give me 10 more retention lines to hold viewers after the hook on a ${NW(P).n} reel.`},
    {id:"payoff",q:"Pick your payoff (the value, 10–35s)",kind:"select",pool:(a,P)=>buildPool(PAYOFF_T,P),ai:(a,P)=>`Give me 10 more ways to deliver the value/payoff in a ${NW(P).n} reel.`},
    {id:"cta",q:"Pick your CTA",kind:"select",pool:(a,P)=>ctaPool(P,txt(a.goal)),ai:(a,P)=>`Give me 10 more CTAs for a ${NW(P).n} reel built to ${txt(a.goal).toLowerCase()}.`},
  ],assemble:(a,P)=>{const{N,W,n}=NW(P);const hook=txt(a.hook),retain=txt(a.retain),payoff=txt(a.payoff),cta=txt(a.cta),g=txt(a.goal);
    const script=`HOOK (0–3s):\n"${hook}"\n\nRETAIN (3–10s):\n${retain}\n\nPAYOFF (10–35s):\n${payoff}\n\nCTA (35s+):\n${cta}`;
    const ost=`① 0:00–0:03 · TOP THIRD\n   Text: "${hook}"\n   Font: bold condensed sans (Montserrat Black / Anton) · Size: 46–52pt · Color: white with a 3px black stroke\n   Motion: pop in on frame 1, slight scale-up. Keep it 2 lines max.\n\n② 0:03–0:10 · CENTER\n   Text: the core of your retain line — one short phrase\n   Font: clean bold sans · Size: 40pt · Color: white + soft drop shadow\n\n③ 0:10–0:35 · LOWER THIRD (one at a time)\n   Text: each payoff point as its own caption\n   Font: same family · Size: 34–36pt · Color: white; highlight the key word in your accent color (e.g. electric blue)\n\n④ Last 2s · CENTER\n   Text: "${cta}"\n   Font: bold · Size: 44pt · Color: white on a subtle dark bar for contrast`;
    const cap1=`${hook}\n\n${payoff}\n\n${cta}`;
    const cap2=`Read this if ${FRAG.pain[0]} 👇\n\n${payoff}\n\nThe difference between creators who ${FRAG.desire[0]} and the ones stuck? Consistency with a real system.\n\n${cta}`;
    const cap3=`${hook}\n\nHere's the truth about ${n}:\n\n${payoff}\n\nSave this. Then go run it.\n\n${cta}`;
    const audio=g==="Sell an offer"||g==="Drive DMs"?"Your voice is the audio. Add a quiet trending track at ~15% under it.":"Use a fast-rising trending sound (under ~50k uses). Tap the ↗ arrow to find them.";
    const kit=`${cap1}\n\n${hashtags(P).join(" ")}`;
    return{title:`Your ${N} reel · built for ${g.toLowerCase()}`,blocks:[
      {label:"Full reel script",text:script},
  ...
  caption:{title:"Caption Lab",intro:"Set the vibe, scroll the options, pick your caption.",steps:[
    ...
    {id:"pick",q:"Pick your caption",kind:"select",pool:(a,P)=>{const{n,W}=NW(P);const tk=P.take||"Most people don't need more motivation. They need a system.";return buildPool([
      `${tk}\n\nThe ones who win in ${n} aren't more talented — they're more consistent.\n\nSave this. Comment "${W}" and I'll send you mine.`,
      `I almost quit ${n} early.\n\nThen I changed one thing: I stopped posting for the algorithm and started posting for the save.\n\nComment "${W}" to get the system.`,
      `Unpopular opinion: posting more isn't the answer in ${n}.\n\nPosting better hooks is.\n\nAgree? Drop a 🔥.`,
      `Nobody talks about this part of ${n}.\n\nThe work that moves the needle is boring and repeatable.\n\nSave this and go run it.`,
      `If {pain}, read this.\n\nIt's not talent. It's a system. And systems are learnable.\n\nComment "${W}" for mine.`,
      `Here's what {time} of consistency in ${n} actually looks like.\n\nNot viral. Just relentless.\n\nSave this for when you want to quit.`,
      `The {n} advice that {result} for me:\n\nStop trying to go viral. Start being impossible to scroll past.\n\nComment "${W}".`],P);},
  ...
  story:{title:"Story Sequencer",intro:"Pick the goal and length — I'll sequence the slides.",steps:[
    {id:"goal",q:"What's the goal?",kind:"select",options:()=>[O("Engagement","Polls & questions"),O("Sell","Bridge to an offer"),O("Authority","Build trust"),O("Launch","Tease & open")]},
    {id:"len",q:"How many slides?",kind:"select",options:()=>["3 slides","5 slides","7 slides"].map(x=>O(x))},
  ],assemble:(a,P)=>{const{n}=NW(P);const len=parseInt(txt(a.len))||3;const g=txt(a.goal);
    const lib=[`Hook: most people in ${n} are doing this wrong. (bold text)`,`Quiz: which grows you faster? → poll [A] / [B]`,`Question box: what's your #1 struggle right now?`,`Mini-story: here's what changed it for me.`,`Proof: a result/screenshot that backs it up.`,`Soft bridge: "want the full system?" → poll [yes] / [later]`,`CTA: tap to grab it → link / DM keyword`];
    const slides=lib.slice(0,len).map((s,i)=>`Slide ${i+1} — ${s}`).join("\n\n");
    return{title:`${len}-slide story · ${g}`,blocks:[{label:"Story sequence",text:slides}],kit:slides};}},
  ...
  bio:{title:"Bio & Link Hub",intro:"Pick your goal and style — get a full profile setup.",steps:[
    ...
    {id:"style",q:"Pick a bio style",kind:"select",pool:(a,P)=>{const{n,N}=NW(P);const k=P.known||`Helping you win at ${n}`;return buildPool([
  ...
  bridge:{title:"Cross-Platform Bridge",intro:"Pick source and target — get a native re-cut plan.",steps:[
    {id:"from",q:"Re-cut from which platform?",kind:"select",options:(a,P)=>(P.platforms&&P.platforms.length?P.platforms:["Instagram"]).map(x=>O(x))},
    {id:"to",q:"To which platform?",kind:"select",options:(a,P)=>{const from=txt(a.from);return PLATFORMS.filter(x=>x!==from).map(x=>O(x));}},
  ],assemble:(a,P)=>{const from=txt(a.from),to=txt(a.to);
    const plan=`New cold open: front-load the payoff in 1.5s — ${to} punishes slow starts.\n\nCaption rewrite: shorter, native phrasing, keyword in line 1 for search.\n\nCTA swap: drive to your bio funnel instead of a comment keyword.\n\nPost timing: ${to}'s peak differs from ${from} — check your active hours.`;
    return{title:`${from} → ${to}`,blocks:[{label:"Re-cut plan",text:plan}],kit:plan};}},
};
...
function docHTML(title,blocks,P,opts){
  ...
  *{box-sizing:border-box}
  ...
  ${body}
  <div class="ft">Produced with CreatorX — the Creator Operating System. Trained by Michael Caso. © ${new Date().getFullYear()}.</div>
  </body></html>`;
}
...
function Builder({P,config,onSaveAsset,askAI}){
  ...
  if(done){
    ...
    return (
      <div style={{padding:22}}>
        <div className="live" style={{marginBottom:12}}><span className="pulse"/>Assembled in {P.name}'s voice</div>
        ...
        <div style={{display:"flex",gap:7,flexWrap:"wrap",marginBottom:16}}>
          ...
          {config.steps.map((s,i)=>(<button key={s.id} className="editchip" onClick={()=>{setDone(false);setSi(i);setPage(0);}}>{s.q.replace(/\?.*/,"").replace(/^(Pick|What|Which|How|Where)\s+(your|the|an?)?\s*/i,"").trim()||s.id} ✎</button>))}
        </div>
        {out.blocks.map((b,i)=>(
          <div className="out-card" key={i}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8,gap:10}}><span className="eyebrow">{b.label}</span><CopyBtn text={b.text}/></div>
            <div className="kit">{b.text}</div>
          </div>
        ))}
        <div style={{marginTop:6,marginBottom:12}}><DocActions title={out.title} blocks={out.blocks} P={P} opts={out.docOpts} label={config===CONFIGS.product?"Generate your product — elite PDF or Word":config===CONFIGS.offer?"Generate Architect Plan — full blueprint doc":"Build this now for me — downloadable doc"}/></div>
        <div className="btn-row" style={{marginTop:2}}>
          ...
          <CopyBtn text={out.kit} label="Copy all"/>
          <button className="btn btn-ghost sm" onClick={()=>{setDone(false);setSi(0);setAns({});setPage(0);}}>Start over</button>
        </div>
      </div>
    );
  }
  ...
  const full=pooled?step.pool(ans,P):(step.options?step.options(ans,P):[]);
  const pageSize=6;
  ...
  const last=si===config.steps.length-1;
  const pick=(o)=>{
  ...
  return (
    <div style={{padding:22}}>
      <div className="bld-prog">{config.steps.map((_,i)=>(<span key={i} className={i<=si?"on":""}/>))}</div>
      <div className="eyebrow" style={{marginTop:14}}>Step {si+1} of {config.steps.length}</div>
      ...
      {pooled && (
        <div style={{display:"flex",gap:9,alignItems:"center",marginTop:13,flexWrap:"wrap"}}>
          <button className="mini" onClick={()=>setPage(p=>p+1)}><RefreshCw size={12}/> More options</button>
          <span className="mono" style={{fontSize:11,color:"var(--faint)"}}>{start+1}–{Math.min(start+pageSize,full.length)} of {full.length}+</span>
      ...
      <div className="btn-row" style={{marginTop:18,justifyContent:"space-between"}}>
        <button className="btn btn-ghost sm" style={{visibility:si===0?"hidden":"visible"}} onClick={()=>{setSi(si-1);setPage(0);}}>Back</button>
        <button className="btn btn-grad sm" disabled={!valid} style={{opacity:valid?1:.4,cursor:valid?"pointer":"not-allowed",display:"flex",gap:7,alignItems:"center"}}
          onClick={()=>{if(!valid)return;last?setDone(true):(setSi(si+1),setPage(0));}}>{last?"Build it":"Next"} <ArrowRight size={15}/></button>
      </div>
    </div>
  );
}

function Drawer({P,k,close,onSave,askAI}){
  ...
  return (
    <>
      ...
      <div className="drawer">
        ...
        {config
          ? <Builder P={P} config={config} onSaveAsset={(t)=>onSave&&onSave(t)} askAI={askAI}/>
...
function videoFrameB64(url){return new Promise((res)=>{try{const v=document.createElement("video");v.crossOrigin="anonymous";v.muted=true;v.src=url;
  v.onloadeddata=()=>{try{v.currentTime=Math.min(1,(v.duration||2)/3);}catch(e){res(null);}};
  v.onseeked=()=>{try{const c=document.createElement("canvas");c.width=v.videoWidth||720;c.height=v.videoHeight||1280;
    c.getContext("2d").drawImage(v,0,0,c.width,c.height);res(c.toDataURL("image/jpeg",.8).split(",")[1]);}catch(e){res(null);}};
  v.onerror=()=>res(null);setTimeout(()=>res(null),6000);}catch(e){res(null);}});}
async function visionAnalyze(P,b64,media,desc){
  const{n}=NW(P);
  const sys=`You are Michael Caso AI, a creator-growth strategist. Analyze the image and return ONLY JSON (no markdown) with keys: type (string, what kind of content this could be), hook (a scroll-stopping opening line), captions (array of 3 full ready-to-post captions in a direct creator voice), reel (array of 4 short steps to build a reel around this), tips (array of 3 posting tips), cta (one CTA). Audience niche: ${n}. ${desc?"Creator's note: "+desc:""}`;
...
function fallbackAnalyze(P,desc){
  const{n,N,W}=NW(P);const d=desc?` about "${desc}"`:"";
  return{type:`${N} content${d} — works as a talking-head reel, a B-roll-over-voiceover, or a carousel.`,
    hook:buildHooks(P)[0].t,
    captions:[`${P.take||"Most people don't need more motivation. They need a system."}\n\nHere's what this ${n} moment taught me.\n\nComment "${W}" for the full breakdown.`,
      `Behind this clip is the real ${n} story.\n\nSave this — it's the part most people skip.\n\nFollow for part 2.`,
      `If ${FRAG.pain[3]}, this is for you.\n\nDon't overthink it — just start. \n\nComment "${W}" and I'll help.`],
    reel:["Open on the strongest 1s of the footage + a bold text hook","Cut to your point — one idea, fast","Layer captions on every line + a trending sound low under your voice","End on your CTA and loop the last line into the first"],
    tips:["Post at your audience's peak hour","Reply to every comment in the first 60 min","Re-share to your story with a poll within the hour"],
    cta:`Comment "${W}" and I'll DM you the breakdown.`};
}
function fallbackCompare(P,a,b){
  const{n}=NW(P);const pick=(rng((a||"a")+(b||"b")+P.name,3)>0.5)?"A":"B";
  return {winner:pick,
    verdict:`Reel ${pick} is the stronger post. It reads as the clearer, faster idea — and in ${n}, clarity in the first second wins. Lead with ${pick}, then re-cut the other as a follow-up.`,
    a:["Hook lands faster","Cleaner single idea","Better thumbnail-stop potential"],
    b:["More detail / depth","Better for a warm audience","Stronger as a part 2"],
    hook:buildHooks(P)[0].t,
    direction:`Sharpen the winner's first 1.5s: open on the boldest visual + a text hook. Cut any intro. Put the payoff promise on screen immediately, and loop the last line into the first so it replays.`};
}
function AnalyzerView({P,askAI}){
  ...
  const analyze=async()=>{setLoading(true);
    try{
      const note=mode==="makeme"?`Adapt this to MY voice and niche (${P.niche}); tell me what to change to make it feel like me. ${desc}`:desc;
      if(file&&file.type.startsWith("image")){const b64=await fileToB64(file.file);setOut(await visionAnalyze(P,b64,file.type,note));}
      else if(file&&file.type.startsWith("video")){const fb=await videoFrameB64(file.url);
        if(fb){try{setOut(await visionAnalyze(P,fb,"image/jpeg",note));}catch(e){setOut(fallbackAnalyze(P,note));}}else setOut(fallbackAnalyze(P,note));}
      else setOut(fallbackAnalyze(P,note));
    }catch(e){setOut(fallbackAnalyze(P,desc));}
    setLoading(false);
  };
  const compare=async()=>{setLoading(true);await new Promise(r=>setTimeout(r,500));setCmp(fallbackCompare(P,file&&file.name,fileB&&fileB.name));setLoading(false);};
  ...
  const title=mode==="post"?"Already filmed it? Let's make it post.":mode==="makeme"?"Saw a reel you love? Make it yours.":"Got two? I'll tell you which to post.";
  const sub=mode==="post"?"Upload a clip or photo (or shoot one now). I'll give you the hook, captions, the reel to build around it, and what'll make it perform."
    :mode==="makeme"?"Upload someone else's reel (or your rough cut). I'll re-work the hook, caption and format so it fits your voice and niche — without copying."
    :"Upload two reels. I'll tell you which one to post, why, and how to sharpen the winner's hook.";
  return (
    <>
      ...
      <p style={{color:"var(--muted)",fontSize:14.5,maxWidth:600,marginBottom:14}}>{sub}</p>
      ...
      {mode!=="winner" ? (
        <div style={{display:"grid",gridTemplateColumns:file?"320px 1fr":"1fr",gap:18}} className="hero">
          <div>
            ...
            :<div><Up f={file}/><input className="cx-in" placeholder="Add context (optional): what's in it / your goal" value={desc} onChange={e=>setDesc(e.target.value)} style={{margin:"10px 0"}}/>
              <div className="btn-row"><button className="btn btn-grad" style={{flex:1,justifyContent:"center"}} disabled={loading} onClick={analyze}>{loading?"Analyzing…":<><Sparkles size={16}/> {mode==="makeme"?"Make it mine":"Analyze & build my post"}</>}</button><button className="btn btn-ghost sm" onClick={reset}>Change</button></div></div>}
          </div>
          <div>
            {!out&&!loading&&<div className="panel panel-pad" style={{color:"var(--muted)",fontSize:14,display:file?"block":"none"}}>Hit analyze and I'll turn this into a ready-to-post plan.</div>}
            {loading&&<div className="panel panel-pad"><div className="live"><span className="pulse"/>Reading the footage…</div></div>}
            {out&&(<div>
              {mode==="makeme"&&<div className="nba" style={{marginBottom:12}}><p style={{fontSize:14,fontWeight:500}}>Here's that idea, re-worked in your voice for {P.niche} — fresh hook, your caption, your format. Not a copy.</p></div>}
              <div className="out-card"><div className="eyebrow" style={{marginBottom:7}}>What this is</div><p style={{fontSize:14,lineHeight:1.5}}>{out.type}</p></div>
              <div className="out-card"><div style={{display:"flex",justifyContent:"space-between",marginBottom:7}}><span className="eyebrow">{mode==="makeme"?"Your hook":"Hook"}</span><CopyBtn text={out.hook}/></div><p style={{fontSize:14.5,fontWeight:500,lineHeight:1.45}}>{out.hook}</p></div>
              ...
              <div className="out-card"><div className="eyebrow" style={{marginBottom:8}}>Build the reel</div>{(out.reel||[]).map((s,i)=>(<div key={i} style={{display:"flex",gap:10,fontSize:14,padding:"6px 0",borderBottom:i<(out.reel.length-1)?"1px solid var(--line)":"none"}}><span className="mono" style={{color:"var(--accent)"}}>{i+1}</span>{s}</div>))}</div>
              <div className="out-card"><div className="eyebrow" style={{marginBottom:8}}>Posting tips</div>{(out.tips||[]).map((s,i)=>(<div key={i} style={{display:"flex",gap:9,fontSize:13.5,padding:"5px 0",color:"var(--muted)"}}><Check size={15} color="var(--green)"/>{s}</div>))}</div>
              <DocActions title="My reel plan" P={P} blocks={[{label:"What this is",text:out.type},{label:"Hook",text:out.hook},...(out.captions||[]).map((c,i)=>({label:"Caption "+(i+1),text:c})),{label:"Reel build",text:(out.reel||[]).join("\n")},{label:"CTA",text:out.cta}]}/>
            </div>)}
          </div>
        </div>
      ):(
        <div>
          ...
          <div className="btn-row" style={{marginBottom:14}}><button className="btn btn-grad" disabled={!file||!fileB||loading} style={{opacity:file&&fileB?1:.5}} onClick={compare}>{loading?"Comparing…":<><Trophy size={16}/> Tell me which to post</>}</button>{(file||fileB)&&<button className="btn btn-ghost sm" onClick={reset}>Reset</button>}</div>
          {cmp&&(<div>
            <div className="nba" style={{marginBottom:12}}><div style={{display:"flex",gap:10,alignItems:"center",marginBottom:6}}><Trophy size={18} color="var(--amber)"/><span className="disp" style={{fontWeight:700,fontSize:18}}>Post Reel {cmp.winner}</span></div><p style={{fontSize:14,lineHeight:1.5}}>{cmp.verdict}</p></div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}} className="hero">
              <div className="out-card"><div className="eyebrow" style={{marginBottom:8}}>Reel A — strengths</div>{cmp.a.map((x,i)=>(<div key={i} style={{display:"flex",gap:8,fontSize:13.5,padding:"4px 0"}}><Check size={14} color="var(--green)"/>{x}</div>))}</div>
              <div className="out-card"><div className="eyebrow" style={{marginBottom:8}}>Reel B — strengths</div>{cmp.b.map((x,i)=>(<div key={i} style={{display:"flex",gap:8,fontSize:13.5,padding:"4px 0"}}><Check size={14} color="var(--green)"/>{x}</div>))}</div>
            </div>
            <div className="out-card"><div style={{display:"flex",justifyContent:"space-between",marginBottom:7}}><span className="eyebrow">Hook direction for the winner</span><CopyBtn text={cmp.direction}/></div><p style={{fontSize:14,lineHeight:1.5}}>{cmp.direction}</p></div>
            <div className="out-card"><div style={{display:"flex",justifyContent:"space-between",marginBottom:7}}><span className="eyebrow">Try this hook</span><CopyBtn text={cmp.hook}/></div><p style={{fontSize:14.5,fontWeight:500}}>{cmp.hook}</p></div>
          </div>)}
        </div>
      )}
    </>
