...
function buildBlueprint(P,q){
  ...
  const become=txt(q.become)||knownPhrase(P);
  const help=(q.help&&q.help.trim())?q.help.trim():`help people ${pk.desires[0]}`;
  const want=(q.build||[]).map(txt).join(", ")||"a personal brand and a first offer";
  const plat=txt(q.platform)||((P.platforms&&P.platforms[0])||"Instagram");
  const vision=txt(q.vision)||"a profitable, recognized brand";
  ...
  // 30-day sample calendar (15 entries rotating pillars + hooks)
  ...
  const ladder=[
    ...
    [`Low-ticket ($27–$97)`,`A ${pk.topics[1]} mini-product or template. Easy yes, filters buyers.`],
    [`Core offer ($297–$997)`,`${_cap(offerName)} — your main transformation. Where most revenue comes from.`],
    [`High-ticket ($1.5k+)`,`1:1 or premium group for people who want speed and access.`],
  ];
  const blocks=[
    {label:"01 · Executive summary",text:`${P.name}, this is your complete brand and growth blueprint for ${N}.\n\nWho you are: someone building ${want}, on track to ${vision}.\nWhat you do: ${help}.\nWho you're becoming: ${become}.\n\nThis document is your operating system — positioning, voice, content, growth, monetization and automation — sequenced into a plan you can run starting today. Everything here is built around one idea: turn attention into a real business.`},
    {label:"02 · Brand positioning statement",text:`"I ${help} — without ${pk.mistakes[0]}."\n\nYou are positioned as ${become}. In a sea of generic ${n} content, your edge is a clear point of view, a repeatable system, and proof. Own one specific outcome and become known for it.`},
    {label:"03 · Your audience avatar",text:`Who you serve:\n• They struggle with: ${pk.pains[0]}, ${pk.pains[1]}\n• They secretly want: ${pk.desires[0]}\n• They've tried: generic advice that didn't stick\n• They follow you because: you make it simple and you've done it\n\nSpeak to ONE person. Every post should make that person feel seen.`},
    {label:"04 · Brand voice & tone",text:`Your voice: ${(P.tone||["Direct","Real"]).join(", ")}.\nRules:\n• Short lines. One idea per post.\n• Lead with the boldest statement.\n• Talk like a person, not a brand.\n• Back claims with proof or story.\n• Always end with a clear next step.`},
    {label:"05 · Content pillars",text:pillars.map((p,i)=>`${i+1}. ${p[0]} — ${p[1]}`).join("\n\n")+`\n\nRotate these. Aim: ~40% authority, 20% story, 20% proof, 10% contrarian, 10% behind-the-scenes.`},
    {label:"06 · Your hook bank (use & remix these)",text:hooks.slice(0,12).map((h,i)=>`${i+1}. ${h.t}`).join("\n")},
    {label:"07 · Signature content formats",text:pk.formats.map((f,i)=>`${i+1}. ${f[0]} — ${f[1]}`).join("\n")},
    {label:"08 · The reel framework",text:`Every reel = 4 beats:\nHOOK (0–3s): stop the scroll. Boldest line first.\nRETAIN (3–10s): "here's what nobody tells you…" keep them.\nPAYOFF (10–35s): deliver one clear, screenshot-worthy idea.\nCTA (35s+): one ask — comment a keyword, save, or follow.\n\nCut every 1.5–2s. Caption every word. Loop the last line into the first.`},
    {label:"09 · 30-day content calendar (first 15 days)",text:cal.join("\n\n")+`\n\n(Days 16–30 repeat the rotation with fresh hooks from your bank.)`},
    {label:"10 · Story sequence strategy",text:`Stories are where trust and sales happen. A 3-slide sequence around each reel:\n1. Tease: "Most people get ${n} wrong…"\n2. Poll: "Want the fix?" → [Yes] [Tell me more]\n3. Drive: "New post is live 👆 go watch."\n\nRun a sales story 2–3x/week: value → proof → soft CTA to your offer.`},
    {label:"11 · Bio & profile optimization",text:`Formula: [who you help] + [outcome] + [proof] + [CTA].\nExample: "${_cap(help)} | ${N} | ${fmt(fol)}+ growing | Free guide 👇"\n\nProfile: clear photo, highlight covers for Start Here / Proof / Offer, link to your lead magnet.`},
    {label:"12 · Growth plan & algorithm levers",text:`What the algorithm rewards now:\n• Saves + shares > likes. Make reference-worthy and send-to-a-friend content.\n• Watch time. Tight pacing, no dead seconds.\n• Fast early engagement. Post at peak, reply in the first 60 min.\n• Consistency. ${plat} rewards creators who show up.\n\nWhat to avoid: engagement bait, reposting low-quality, buying followers, inconsistent posting.`},
    {label:"13 · Posting cadence",text:`Sustainable, aggressive cadence:\n• ${plat}: 1 reel/day (or 5/week minimum)\n• Stories: daily (3–5 frames)\n• 1 carousel/week for saves\n\nBatch film 1x/week. Never let perfect stop you from posting.`},
    {label:"14 · Monetization model — your offer ladder",text:ladder.map((l,i)=>`${i+1}. ${l[0]} — ${l[1]}`).join("\n\n")+`\n\nAt ${fmt(fol)} followers, start at the lead magnet + core offer. Don't build a course yet — sell ${offerName} and close in DMs first.`},
    {label:"15 · Your first offer (architected)",text:`Offer: ${_cap(offerName)}\nPromise: ${_cap(pk.desires[0])} in 60 days.\nFor: people who ${pk.pains[0]}.\nIncludes: a clear system, accountability, and direct access.\nPrice: start $297–$497, raise as proof grows.\nGuarantee: show up and do the work, or your money back.`},
    {label:"16 · DM sales flow",text:`When someone shows interest:\n1. Connect — ask about their goal.\n2. Qualify — "what have you tried?"\n3. Diagnose — name the gap.\n4. Offer — "here's how I'd help."\n5. Close — send the link / take payment.\n6. Follow up — once, with value, if they go quiet.`},
    {label:"17 · Automation funnel",text:`Turn content into customers on autopilot:\nKEYWORD (comment) → AUTO-DM (link + lead magnet) → LANDING (capture email) → EMAIL SEQUENCE (nurture) → BOOKING/CHECKOUT → CLOSE.\n\nTools: ManyChat (comment→DM), your site/Stan/Beacons (capture), an email tool (nurture), Stripe (payment).`},
    {label:"18 · Email sequence outline",text:`Welcome sequence (5 emails):\n1. Welcome + fastest win\n2. The #1 mistake (${pk.mistakes[0]})\n3. Your story / proof\n4. The framework\n5. The invitation to ${offerName}\n\nSend days 0,1,3,5,7. This is where social attention becomes owned audience.`},
    {label:"19 · KPIs — what to track weekly",text:`• Posts shipped\n• Followers gained\n• Avg views & watch %\n• Saves + shares (your reach engine)\n• Profile visits → follows\n• DMs / leads\n• Revenue\n\nNorth star: publish-ready assets shipped per week. Consistency compounds.`},
    {label:"20 · Your 90-day roadmap",text:`DAYS 1–30 — Foundation: lock positioning, bio, pillars. Post daily. Launch lead magnet.\nDAYS 31–60 — Momentum: double down on your best format, start the email list, soft-launch ${offerName}.\nDAYS 61–90 — Monetize: run a real launch, raise prices with proof, turn on automation.\n\nTarget: ${fmt(Math.round(fol*1.6))}+ followers and your first ${money(Math.max(1000,(P.revTarget||2000)))}/mo.`},
    {label:"21 · What to do / what NOT to do",text:`DO:\n✓ Pick one outcome and own it\n✓ Post consistently, hook hard, deliver value\n✓ Build the email list from day one\n✓ Sell in DMs before building anything big\n✓ Track saves & shares\n\nDON'T:\n✗ Chase every trend with no point of view\n✗ Wait for "perfect" to post\n✗ Build a course before you've sold the offer manually\n✗ Ignore your DMs — that's where money is\n✗ Buy followers or engagement`},
    {label:"22 · Your next 7 days",text:`Day 1: Fix your bio with the formula above.\nDay 2: Generate 25 hooks, pick 5.\nDay 3: Film 3 reels (batch).\nDay 4: Build your lead magnet.\nDay 5: Post + write your welcome email 1.\nDay 6: Set up the comment→DM automation.\nDay 7: Review the week, log your numbers, plan next week.\n\nRun this in CreatorX. Everything in this blueprint has a tool inside the platform.`},
  ];
  return {title:`${P.name}'s Brand Blueprint`,subtitle:`${N} · built to reach ${vision}`,blocks,
    docOpts:{subtitle:`A complete brand, content, growth & monetization system for ${P.name} — ${N}.`}};
}
function BrandBlueprintView({P,go,open}){
  const [q,setQ]=useState({help:"",become:null,build:[],platform:null,vision:null});
  ...
  const platOpts=(P.platforms&&P.platforms.length?P.platforms:["Instagram","TikTok","YouTube","LinkedIn"]);
  const visionOpts=["A profitable side income","A full-time creator business","A 6-figure brand","A 7-figure company","Just to grow & be known"];
  const ready=q.become&&q.build.length&&q.platform&&q.vision;
  const tog=(v)=>setQ(s=>({...s,build:s.build.includes(v)?s.build.filter(x=>x!==v):[...s.build,v]}));
  const generate=()=>{setBuilding(true);setTimeout(()=>{setReport(buildBlueprint(P,q));setBuilding(false);},900);};
  if(report){
    return (
      <>
        ...
        <div className="panel panel-pad" style={{marginBottom:16,background:"var(--grad-soft)",border:"1px solid rgba(79,140,255,.3)"}}>
          <div style={{display:"flex",gap:12,alignItems:"center"}}><div className="tool-ic" style={{width:38,height:38}}><FileText size={18}/></div>
            <div><div style={{fontWeight:700,fontSize:15}}>Your complete {report.blocks.length}-section blueprint is ready.</div><div style={{fontSize:13,color:"var(--muted)"}}>Positioning, content, growth, monetization & automation — branded and ready to follow.</div></div></div>
  ...
  return (
    <>
      <div className="eyebrow">Brand Blueprint</div>
      <h1 className="disp" style={{fontWeight:700,fontSize:28,margin:"6px 0 6px"}}>Tell me who you want to become. I'll build the whole plan.</h1>
      <p style={{color:"var(--muted)",fontSize:14.5,maxWidth:640,marginBottom:20}}>Answer five quick things and I'll generate a complete, branded blueprint — your positioning, content pillars, hook bank, 30-day calendar, offer ladder, automation funnel, 90-day roadmap and more. Everything in one elite document you can follow step by step.</p>

      <div className="panel panel-pad" style={{marginBottom:14}}>
        <div className="eyebrow" style={{marginBottom:8}}>1 · In one line — who do you help and how? <span style={{color:"var(--faint)",textTransform:"none",letterSpacing:0}}>(optional)</span></div>
        <input className="cx-in" placeholder={`e.g. I help busy people ${pack(P).desires[0]}`} value={q.help} onChange={e=>setQ(s=>({...s,help:e.target.value}))}/>
      </div>
      <div className="panel panel-pad" style={{marginBottom:14}}>
        <div className="eyebrow" style={{marginBottom:10}}>2 · Who do you want to become?</div>
        <div className="opts two">{becomeOpts.map(o=>(<div key={o} className={"chip"+(txt(q.become)===o?" on":"")} onClick={()=>setQ(s=>({...s,become:O(o)}))}><span className="dot"/>{o}{txt(q.become)===o&&<Check size={14} className="ck"/>}</div>))}</div>
      </div>
      <div className="panel panel-pad" style={{marginBottom:14}}>
        <div className="eyebrow" style={{marginBottom:10}}>3 · What do you want to build? <span style={{color:"var(--faint)",textTransform:"none",letterSpacing:0}}>(pick any)</span></div>
        <div className="opts two">{buildOpts.map(o=>(<div key={o} className={"chip"+(q.build.includes(o)?" on":"")} onClick={()=>tog(o)}><span className="dot"/>{o}{q.build.includes(o)&&<Check size={14} className="ck"/>}</div>))}</div>
      </div>
      <div className="panel panel-pad" style={{marginBottom:14}}>
        <div className="eyebrow" style={{marginBottom:10}}>4 · Main platform?</div>
        <div className="opts two">{platOpts.map(o=>(<div key={o} className={"chip"+(txt(q.platform)===o?" on":"")} onClick={()=>setQ(s=>({...s,platform:O(o)}))}><span className="dot"/>{o}{txt(q.platform)===o&&<Check size={14} className="ck"/>}</div>))}</div>
      </div>
      <div className="panel panel-pad" style={{marginBottom:18}}>
        <div className="eyebrow" style={{marginBottom:10}}>5 · 12-month vision?</div>
        <div className="opts two">{visionOpts.map(o=>(<div key={o} className={"chip"+(txt(q.vision)===o?" on":"")} onClick={()=>setQ(s=>({...s,vision:O(o)}))}><span className="dot"/>{o}{txt(q.vision)===o&&<Check size={14} className="ck"/>}</div>))}</div>
      </div>
      <button className="btn btn-grad" style={{width:"100%",justifyContent:"center",padding:"15px",fontSize:15.5,opacity:ready?1:.5,cursor:ready?"pointer":"not-allowed"}} disabled={!ready||building} onClick={generate}>
        {building?<><span className="pulse" style={{marginRight:8}}/>Building your blueprint…</>:<><Sparkles size={18}/> Generate my Brand Blueprint</>}
      </button>
    </>
  );
}
...
function retainPool(a,P){const{n}=NW(P);const pk=pack(P);
  const bases=[
    "But here's what nobody actually tells you 👇",
    "Stick with me — most people quit right before the part that matters.",
    "And no, it's not what every guru repeats.",
    "Here's the part that changed everything for me.",
    "Let me show you exactly why — fast.",
    `Because the real reason ${pk.pains[0]} is simpler than you think.`,
    "Watch this part twice — it's the whole game.",
    "I wish someone told me this two years ago.",
    "The fix takes 30 seconds to understand.",
    `Most people in ${n} get this completely backwards.`,
    "Save this before it gets buried.",
    "This is where 90% mess it up 👇",
    `If ${pk.pains[1]}, this next part is for you.`,
    "Quick story before the how —",
    "Don't scroll yet, the third one matters most.",
  ];
  return shuffleSeeded(bases,(P.name||"")+daySeed()+(txt(a.hook)||"")+"ret");
}
function payoffPool(a,P){const{n}=NW(P);const pk=pack(P);const out=[];
  pk.topics.forEach(t=>out.push(`The move: ${t}.\n• Start small and specific\n• Stay consistent for two weeks\n• Track it and adjust`));
  pk.desires.forEach((d,i)=>out.push(`Want to ${d}? Do these 3 things:\n1) ${_cap(pk.topics[i%pk.topics.length])}\n2) Cut what isn't working\n3) Double down on what is`));
  pk.mistakes.forEach((m,i)=>out.push(`Stop ${m}. Instead:\n→ ${_cap(pk.topics[i%pk.topics.length])}\n→ Give it two honest weeks\n→ Watch what changes`));
  pk.results.forEach((r,i)=>out.push(`Here's how ${r.toLowerCase()}:\n→ ${_cap(pk.topics[i%pk.topics.length])}\n→ Then ${pk.desires[i%pk.desires.length]}\n→ Repeat until it's automatic`));
...
function buildReel(a,P){
  ...
  const cap1=`${hook}\n\n${payoff}\n\n${cta}`;
  const cap2=`Read this if ${pk.pains[0]} 👇\n\n${payoff}\n\nThe difference between people who ${pk.desires[0]} and the ones stuck? A real system.\n\n${cta}`;
...
const CONFIGS={
  ...
  caption:{title:"Caption Lab",intro:"Set the vibe, scroll the options, pick your caption.",steps:[
    ...
    {id:"pick",q:"Pick your caption (Refresh for thousands more — then edit it)",kind:"select",pool:(a,P)=>{const{n,N,W}=NW(P);const tk=takeLine(P);return buildPool([
      ...
      `Stop {mistake}.\n\nIt's quietly killing your results in ${n}. Do this instead 👇\n\nSave it.`,
      `Most people in ${n} want {desire}.\n\nFew are willing to do the unsexy work that gets them there.\n\nWhich one are you? Comment below.`,
      `${_cap("{topic}")} changed everything for me in ${n}.\n\nHere's the version I wish I had on day one.\n\nComment "${W}" and it's yours.`,
      `Read this if you keep {pain}.\n\nThe problem isn't effort. It's direction.\n\nSave this and fix it this week.`,
      `3 things I'd tell anyone starting ${n} today:\n\n1. ${_cap("{topic}")}\n2. Be consistent before you're clever\n3. Sell before you scale\n\nSave for later 📌`,
      `I went from invisible to {result} in ${n}.\n\nNot luck. A system anyone can run.\n\nWant it? Comment "${W}".`,
      `The hard truth about ${n}:\n\nYou don't need more motivation. You need {topic} and a calendar.\n\nDrop a 🔥 if you needed this.`,
      `Everyone told me to {mistake}.\n\nI did the opposite and {result}.\n\nSometimes the crowd is the warning. Save this.`,
      `${tk}\n\nSave this one — it's the part most people skip.\n\nComment "${W}" for the full breakdown.`,
      `You're not behind in ${n}.\n\nYou're just one system away.\n\nComment "${W}" and I'll send you exactly what I use.`,
      `What {time} in ${n} taught me:\n\nThe goal isn't to be everywhere. It's to be unforgettable somewhere.\n\nSave it.`,
      `Hot take: ${n} isn't saturated.\n\nIt's full of people quitting right before it works.\n\nDon't be one of them. Follow for the push.`,
      `If you want to {desire}, stop {mistake}.\n\nThat's it. That's the post.\n\nSave it and go.`,
      `The 1% in ${n} do this differently:\n\nThey treat content like a system, not a lottery ticket.\n\nComment "${W}" for the system.`,
      `Nobody's coming to save your ${n} account.\n\nGood news: you don't need them to.\n\nHere's where to start 👇 Save it.`,
      `I used to {mistake}.\n\nNow I {result}.\n\nThe difference was one decision. Comment "${W}" and I'll tell you.`,
      `${_cap("{topic}")} is the most underrated move in ${n}.\n\nDo it for 30 days and watch.\n\nSave this challenge.`,
      `Your ${n} content isn't the problem.\n\nYour hook is.\n\nFix the first 3 seconds and everything changes. Save this.`,
      `Real talk: {pain} is a positioning problem, not an effort problem.\n\nSay one thing, to one person, really well.\n\nComment "${W}".`,
      `The fastest way to {desire} in ${n}?\n\nPick one format. Master it. Repeat until it's boring.\n\nSave this and commit.`,
      `${tk}\n\nMost won't do it. The few who do will {result}.\n\nComment "${W}" if you're in.`],P);},
